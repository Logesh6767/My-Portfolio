...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    setIsMobile(mobile);
    // Only run JS-driven parallax on capable desktop devices
    setEnableParallax(!mobile && !reduced);

    const onResize = () => {
      const m = window.innerWidth < 640;
      setIsMobile(m);
      setEnableParallax(!m && !reduced);
    };
  ...
  const y1Raw = useTransform(scrollYProgress, [0, 1], [0, -150]);
  const y2Raw = useTransform(scrollYProgress, [0, 1], [0, -300]);
  const y3Raw = useTransform(scrollYProgress, [0, 1], [0, -200]);
  ...
  return (
    <section ref={ref} id="home" className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      ...
      {/*
        Background shapes — NO blur, NO mix-blend-multiply.
        Simple radial-gradient circles: cost is near-zero, compositor handles them.
        Visual result is similar — soft colored glows.
      */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        <div
          className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full animate-float"
        ...
        />
        <div
          className="absolute top-[40%] right-[30%] w-[300px] h-[300px] rounded-full animate-pulse-slow"
          style={{ background: 'radial-gradient(circle, rgba(249,168,212,0.20) 0%, transparent 70%)' }}
        />
      </div>
      ...
      <div className="container mx-auto px-6 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="relative h-[400px] lg:h-[600px] w-full perspective-1000 mt-12 lg:mt-0">
          ...
          {/* Main Floating Card — no backdrop-blur, solid bg */}
          <motion.div
            style={{ y: enableParallax ? y1 : 0 }}
            className="absolute top-[10%] left-[5%] lg:left-[10%] w-[90%] lg:w-[80%] h-[70%] z-20"
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 30 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              transition={{ duration: 1.2, delay: 0.2, ease: 'easeOut' }}
              className="w-full h-full bg-white/85 border border-white/60 rounded-3xl shadow-2xl animate-float flex items-center justify-center overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/20 to-white/40" />
              <div className="w-[90%] h-[90%] bg-white/60 rounded-2xl border border-white/50 p-6 flex flex-col gap-4">
                ...
                <div className="flex gap-4">
                  <div className="w-1/3 h-24 lg:h-32 bg-blue-100/70 rounded-xl animate-pulse" />
                  <div className="w-2/3 h-24 lg:h-32 bg-purple-100/70 rounded-xl animate-pulse" style={{ animationDelay: '1s' }} />
                </div>
                <div className="w-full h-20 lg:h-24 bg-pink-100/70 rounded-xl animate-pulse" style={{ animationDelay: '2s' }} />
                <div className="flex-1 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl" />
              </div>
            </motion.div>
          </motion.div>

          {/* Top-right floating badge — no backdrop-blur */}
          <motion.div
          ...
          >
            <motion.div
              ...
              className="p-4 bg-white/95 rounded-2xl shadow-xl animate-float-delayed ring-1 ring-gray-100"
            >
          ...
          {/* Bottom-left floating badge — no backdrop-blur */}
          <motion.div
          ...
          >
            <motion.div
              ...
              className="p-4 bg-white/95 rounded-2xl shadow-xl animate-float ring-1 ring-gray-100"
